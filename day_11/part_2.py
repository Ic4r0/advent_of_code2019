...
class Hull:
    ...
    def print_reg_id(self):
        current_view = {}
        for elem in self.panels_steps:
            color, coords = elem
            current_view[coords] = color
        min_x = min(x for x, _ in list(current_view))
        min_y = min(y for _, y in list(current_view))
        max_x = max(x for x, _ in list(current_view))
        max_y = max(y for _, y in list(current_view))

        output_string = ''
        for x in range(min_x, max_x+1):
            for y in range(min_y, max_y+1):
                if (x, y) in list(current_view):
                    if current_view[(x, y)] == 1:
                        output_string += 'O'
                    else:
                        output_string += ' '
                else:
        ...
        output_path, _ = os.path.split(os.path.abspath(__file__))
        with open(output_path + '\\day_11_part_2', 'w') as output_file:
...
ship_hull = Hull(starting_color=1)
